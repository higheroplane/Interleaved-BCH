...
namespace galois
{
   ...
   GaloisFieldPolynomial& GaloisFieldPolynomial::operator/=(const GaloisFieldPolynomial& divisor)
   {
      ...
      {
            for (int i = 0; i < poly.size(); i ++) poly[i] /= divisor[0];
      }
      else printf ("sooosad\n");
      return *this;
   }
   ...
   GaloisFieldPolynomial& GaloisFieldPolynomial::operator%=(const GaloisFieldPolynomial& divisor)
   {
         ...
         )
      {
         ...
         if (divisor.deg () == 0) poly = std::vector<GaloisFieldElement>(1, GaloisFieldElement (gf, 0));
         else
         {
            for(int i = deg(); i >= 0; i--)
            {
   ...
   void GaloisFieldPolynomial::simplify(GaloisFieldPolynomial& polynomial)
   {
      if (poly.size() > 0)
      {
         ...
         while((last >= 0) && (polynomial.poly.size() > 1))
         {
...
void gcdext (GaloisFieldPolynomial  a,  GaloisFieldPolynomial  b, GaloisFieldPolynomial *d,
             GaloisFieldPolynomial *x,  GaloisFieldPolynomial *y)
{
   assert (d);
   assert (x);
   assert (y);
   ...
   galois::GaloisFieldPolynomial s (gf, 0);
...
GaloisFieldPolynomial key_equation (GaloisFieldPolynomial t, GaloisFieldPolynomial s, int d)
{
   assert (t.field() == s.field());
   GaloisField * gf = t.field();
   galois::GaloisFieldElement z[1] = {galois::GaloisFieldElement (gf, 0)};
   galois::GaloisFieldElement o[1] = {galois::GaloisFieldElement (gf, 1)};
   galois::GaloisFieldPolynomial zero (gf, 0, z);
   galois::GaloisFieldPolynomial one  (gf, 0, o);
   galois::GaloisFieldPolynomial temp  (gf, 0, o);

  // GaloisFieldPolynomial matrix [4] = {one, zero, one, zero};

   GaloisFieldPolynomial Q = zero, R = zero;
   GaloisFieldPolynomial A [2][2] = {one, zero, zero, one};
   GaloisFieldPolynomial T [2][2] = {one, zero, zero, one};
   ...
   while (t.deg() >= d/2)
   {
      ...
      T [0][0] = A [0][0]; T [0][1] = A [0][1];
      T [1][0] = A [1][0]; T [1][1] = A [1][1];
